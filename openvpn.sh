...
prefetch: yes' >>/etc/unbound/unbound.conf
			...
			# Configuration
			sed -i 's|# interface: 0.0.0.0$|interface: 192.168.31.0|' /etc/unbound/unbound.conf
			sed -i 's|# access-control: 127.0.0.0/8 allow|access-control: 192.168.31.0/24 allow|' /etc/unbound/unbound.conf
...
function installQuestions() {
	...
	echo "The git repository is available at: https://github.com/aidan-lemay/setup-scripts"
	echo ""
...
ifconfig-pool-persist ipp.txt" >>/etc/openvpn/server.conf
	...
	2) # Self-hosted DNS resolver (Unbound)
		echo 'push "dhcp-option DNS 192.168.31.0"' >>/etc/openvpn/server.conf
		if [[ $IPV6_SUPPORT == 'y' ]]; then
...
function newClient() {
	...
	if [ -e "/home/${CLIENT}" ]; then
		# if $1 is a user name
		homeDir="/home/${CLIENT}/openvpn"
	elif [ "${SUDO_USER}" ]; then
		# if not, use SUDO_USER
		if [ "${SUDO_USER}" == "root" ]; then
			# If running sudo as root
			homeDir="/root/openvpn"
		else
			homeDir="/home/${SUDO_USER}/openvpn"
		fi
	else
	...
	cp /etc/openvpn/client-template.txt "$homeDir/openvpn/$CLIENT.ovpn"
	{
		echo "<ca>"
	...
	} >>"$homeDir/openvpn/$CLIENT.ovpn"

	echo ""
	echo "The configuration file has been written to $homeDir/openvpn/$CLIENT.ovpn."
